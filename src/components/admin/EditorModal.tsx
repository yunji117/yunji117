...
export default function EditorModal({ title, headerActions, onClose, children }: { title: string; headerActions?: ReactNode; onClose: () => void; children: ReactNode }) {
  ...
  return <div className="fixed inset-0 z-[90] overflow-y-auto bg-black/60 p-3 backdrop-blur-sm sm:p-6" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-label={title} className="relative mx-auto max-w-3xl rounded-2xl border border-slate-200 bg-[#fffdf8] p-5 text-gray-900 shadow-2xl outline-none dark:border-white/10 dark:bg-slate-900 dark:text-white sm:p-8">
      <header className="mb-6 flex items-center justify-between gap-4"><h2 className="text-lg font-bold">{title}</h2><div className="flex items-center gap-2">{headerActions}<button type="button" onClick={onClose} aria-label="편집 닫기" className="rounded-full p-2 hover:bg-gray-200 dark:hover:bg-white/10"><X /></button></div></header>
      {children}
    </div>
  </div>;
}
